...
new file go back to defaults). A `mobai-cloud mock` call patches one capability
...
- **`appearance` is the colour scheme the app renders in**, `{mode: light}` or
  ...
  applies after the scenario, so the flag wins over the file; `mobai-cloud mock
...
```bash
mobai-cloud preview run --detach --scenario scenarios/checkout-success.yaml --json
```
...
mobai-cloud mock network '{"routes":{"POST /api/orders":{"status":500,"body":{"error":"upstream is down"}}}}' --json
mobai-cloud preview tap --label "Place order" --json
mobai-cloud preview inspect --json     # assert the error state rendered
```
...
```bash
mobai-cloud preview mock-state --json
```
