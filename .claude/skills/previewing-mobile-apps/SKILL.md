...
as missing, do not skip past it. Say what it changes, say the download comes
from Apple under Apple's terms, and if the person agrees, run the `install`
commands it gives (they carry the acceptance as an environment variable), then
check `renderer` again. Do this before the first screenshot you show anyone.
...
engine has no answer for. Point it at the file holding the screen.
